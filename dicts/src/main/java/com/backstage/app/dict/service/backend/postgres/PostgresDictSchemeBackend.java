...
import com.backstage.app.dict.exception.dict.index.IndexDeletedException;
...
public class PostgresDictSchemeBackend extends AbstractPostgresBackend implements DictSchemeBackend
{
	...
	private Dict updatedDictScheme(String dictId, Dict updatedDict)
	{
		...
		var addColumnOperations = updatedDict.getFields()
				...
				.map(it -> {
					...
					return sqlWithParameters("alter table ${scheme}.${dictId} add column ${column} ${definition}", parameterMap);
				})
				.toList();
		...
		var dropColumnOperations = actualDict.getFields()
				...
				.map(it -> {
					...
					return sqlWithParameters("alter table ${scheme}.${dictId} drop column ${column}", parameterMap);
				})
				.toList();
	...
	private DictConstraint createdConstraint(Dict dict, DictConstraint constraint)
	{
		...
		var sql = sqlWithParameters("alter table ${scheme}.${dictId} add constraint ${constraint} unique (${fields})", parameterMap);
	...
	private void deleteDictConstraint(Dict dict, String id)
	{
		...
		var sql = sqlWithParameters("alter table ${scheme}.${dictId} drop constraint ${constraint}", parameterMap);
	...
	private DictIndex createdIndex(Dict dict, DictIndex index)
	{
		...
		var sql = sqlWithParameters("create index ${indexId} on ${scheme}.${dictId} (${fields})", parameterMap);
	...
	private void deleteDictIndex(String indexId)
	{
		...
		var sql = sqlWithParameters("drop index ${scheme}.${indexId}", parameterMap);
