...
package com.backstage.app.dict.service.ddl;

import com.backstage.app.dict.exception.DDLSyntaxException;
import com.backstage.app.dict.service.ddl.ast.*;
import com.backstage.app.dict.service.ddl.ast.expression.*;
import com.backstage.app.dict.service.ddl.ast.expression.table.AlterTable;
import com.backstage.app.dict.service.ddl.ast.expression.table.CreateIndexExpression;
...
public class SqlParser
{
	...
	final Parser<ColumnDefinition> COLUMN_DEFINITION = Parsers.sequence(
			...
			ID,
			term("[]").asOptional(),
			phrase("not", "null").asOptional(),
			Parsers.sequence(term("references"), ID).asOptional(),
			Parsers.sequence(term("default"), VALUE).asOptional(),
			(field, name, type, multivalued, required, reference, defaultValue) -> new ColumnDefinition(field, name, type, multivalued.isPresent(), required.isPresent(), reference.orElse(null), defaultValue.orElse(null)));
