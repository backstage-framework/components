 ...
 * Провайдер должен быть выполнен после выполнения {@link DictLockInitializer}, из-за необходимости
 * обязательного наличия идентификатора справочника в коллекции локов {@link DictLockService}.
 */
@Slf4j
@Component
...
public class DictsDDLProvider
{
	...
	public void update()
	{
		dictLockInitializer.initialize();
		...
		try
		{
			...
			for (Resource resource : resolver.getResources("classpath:" + MIGRATIONS_PATH))
			{
				...
				if (resourceConnection instanceof JarURLConnection jarURLConnection)
				{
					...
					var iterator = file.entries().asIterator();

					StreamSupport.stream(Spliterators.spliteratorUnknownSize(iterator, Spliterator.ORDERED), false).filter(it -> it.getName().startsWith(MIGRATIONS_PATH + SEPARATOR)).filter(it -> it.getName().endsWith(SQL_EXTENSION)).forEach(it -> {
				...
				{
					FileUtils.listFilesAndDirs(resource.getFile(), FalseFileFilter.INSTANCE, DirectoryFileFilter.INSTANCE).stream().flatMap(it -> FileUtils.listFiles(it, new String[]{SQL_EXTENSION}, false).stream()).forEach(file -> {
