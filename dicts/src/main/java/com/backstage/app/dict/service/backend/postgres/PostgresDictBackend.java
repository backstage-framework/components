...
import com.backstage.app.dict.exception.dict.enums.EnumDeletedException;
import com.backstage.app.dict.exception.dict.enums.EnumNotFoundException;
import com.backstage.app.dict.exception.dict.enums.EnumUpdatedException;
...
import com.backstage.app.dict.service.backend.postgres.rowmapper.DictRowMapper;
...
public class PostgresDictBackend extends AbstractPostgresBackend implements DictBackend
{
	...
	{
		var sql = "select * from %s.dict".formatted(dictsProperties.getDdl().getScheme());
	...
	{
		var sql = "select exists(select 1 from %s.dict where id = :id)".formatted(dictsProperties.getDdl().getScheme());
