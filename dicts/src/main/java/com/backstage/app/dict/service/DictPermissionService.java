...
public class DictPermissionService
{
	...
	public void checkViewPermission(Dict dict, String userId)
	{
		...
		{
			throw new AppException(CoreAppStatusCode.ACCESS_RIGHTS_ERROR, "Недоступен просмотр справочника: %s.".formatted(dict.getId()));
		}
	}
	...
	public void checkEditPermission(Dict dict, String userId)
	{
		...
		if (!permissionService.getPermissions(userId).contains(permission))
		{
			throw new AppException(CoreAppStatusCode.ACCESS_RIGHTS_ERROR, "Недоступно изменение справочника: %s.".formatted(dict.getId()));
		}
	}
}
