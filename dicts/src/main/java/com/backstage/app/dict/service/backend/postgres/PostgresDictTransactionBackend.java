...
import com.backstage.app.dict.exception.migration.transaction.DictCopyException;
import com.backstage.app.dict.exception.migration.transaction.DictRollbackException;
import com.backstage.app.dict.model.dict.DictTransactionItem;
	...
	private String copiedDict(String dictId)
	{
		...
		var copySchema = """
				...
				$$
				    ...
				    begin
				        execute format(
				                'create table ${scheme}.%s (like ${scheme}.%s including all)',
				                '${copy}', '${original}');
				        for rec in
				            select oid, conname
				            from pg_constraint
				            where contype = 'f'
				              and conrelid = '${scheme}.${original}'::regclass
				            loop
				                execute format(
				                        'alter table ${scheme}.%s add constraint %s ${scheme}.%s',
				                        '${copy}',
		...
		var insertDataSql = sqlWithParameters("insert into ${scheme}.${copy} (select * from ${scheme}.${original})", parameterMap);
