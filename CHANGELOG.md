## 5.2.33 - 2026-08-07
### Audit
- Исправили ошибку при сохранении записей audit c NULL в значениях fields или properties.
...
### BPM
- Добавили миграции схемы базы данных Camunda в модуле bpm-camunda до версии 7.23.0.
...
## 5.1.71 - 2025-07-28
### Dicts
- Убрали @RequestBody в методах импорта для получения прямого доступа к InputStream, избежав ошибки десериализации Jackson.

## 5.1.70 - 2025-07-14
### Dicts
- Добавили @SuppressWarnings("unused") в сгенерированные модели.
...
## 5.1.66 - 2025-06-19
### Dicts
- Исправили тип исключения при попытке установить в поле справочника некорректное decimal значение.

## 5.1.65 - 2025-06-07
### Dicts
- Исправили ошибку обновления полей, названия которых совпадали с зарезервированными словами для бэкэнда PostgreSQL.

## 5.1.64 - 2025-05-29
### Dicts
- Исправили ошибку валидации данных при использовании нескольких ref-полей, ссылающихся на один справочник.

## 5.1.63 - 2025-05-28
### Dicts
- Исправили миграцию структуры справочников для бэкэнда PostgreSQL.

## 5.1.62 - 2025-05-26
### Dicts
- Для enum теперь генерируется метод toString, это позволяет использовать значения enum для Condition в getByFilter без дополнительных преобразований.
...
- Spring Boot 3.4.6
 ...
 ### Dicts
- В QueryParser добавили поддержку is null / is not null.
- Исправили ошибку, из-за которой не заполнялось значение по умолчанию для enum полей при создании новой записи.
- Исправили ошибку кодогенерации для случая, когда не было задано описание у enum.
...
## 5.1.55 - 2025-04-28
### Report
- Исправили заполнение имени файла при обновлении вложения.
...
## 5.1.51 - 2025-04-14
### Gradle-Plugins
- Расширения gradle перенесены в основной репозиторий, версии синхронизированы с основными зависимостями.

## 5.1.50 - 2025-04-04
### Dicts
- DictService теперь возвращает immutable объекты Dict для предотвращения внутренних ошибок после изменения этих объектов внешним кодом. Для получения mutable версий Dict необходимо использовать метод Dict::copy.

## 5.1.49 - 2025-04-03
### App
- Исправили конфигурацию JsonEncoder для вывода formattedMessage.

## 5.1.48 - 2025-04-02
### Dicts
- Добавили возможность ограничить размер истории для элемента справочника через параметр `app.dicts.maxHistory` или конфигурацию справочника.

## 5.1.47 - 2025-03-31
### Dicts
- Добавили миграцию для удаления soft delete записей в справочниках после перехода на hard delete.
- Доработали условия добавления аннотации @SuppressWarnings в сгенерированные клиенты.

### Jobs
- Больше не требуется переопределять метод createDefaultParams() для корректной работы кастомных параметров задачи.
- Схема параметров задачи теперь возвращается в формате OpenAPI.
...
### BPM
- Добавили миграции схемы базы данных Camunda в модуле bpm-camunda до версии 7.22.0.

### Database
- Добавили поддержку аннотации @EntityScan при инициализации EntityManagerFactory.

### Dicts
- API для получения реальной схемы справочника в конкретном хранилище.
- В истории элемента справочника теперь сохраняем только дельту, а не весь элемент целиком.
- Добавлена обработка сортировки без пагинации в методе dictDataService.getByFilter.
...
- jOOQ 3.19.20
- Flyway 11.3.3
- ClickHouse JDBC 0.8.1
- Groovy 4.0.25

### Api
- Добавили EnumEndpoint для получения описания enum'ов аннотированных @ApiEnum.

### Audit
- Оптимизировали модель хранения.

### Dicts
- Всегда возвращаем для элемента справочника его идентификатор и версию, даже если эти поля не были запрошены.
- Добавили метод dictDataService.streamByFilter.
- Реализовали `DictServiceAdvice` для возможности добавления побочных эффектов в логику `DictService`.
- Добавили возможность экспорта справочника по фильтру.
- Переработали экспорт справочников в SQL, добавлена поддержка JSON и GeoJSON.
- Исправили экспорт справочников в CSV, null поля выгружаются как пустые строки.
- Исправили ошибку при создании элемента справочника с JSON полем в значении null.
- Исправили работу сортировки в бэкэнде PostgreSQL.
- Ускорили вставку и обновление записей справочников для всех бэкэндов.

## 5.1.15 - 2025-02-15
### Dicts
- Восстановили rollback в случае ошибки миграции для Postgres.
- Существенно ускорили вставку элементов в справочник для Postgres.
...
## 5.1.11 - 2025-02-12
### Dicts
- Добавили поддержку множественных полей GeoJSON.
...
## 5.1.4 - 2025-01-20
### Dicts
- Реализовали полноценное удаление для справочников.

## 5.1.3 - 2025-01-20
### Dicts
- Добавили метод getDistinctValuesByFilter в сервисы кодогенерации.

## 5.1.2 - 2025-01-20
### Dicts
- Удалено принудительное проставление движка справочникам при запуске приложения.
- Добавлена возможность изменять таблицу `dict` программно.
...
## 4.5.29 - 2023-08-11
### App
- Добавлена абстракция переопределеного поведения json сериализации/десериализации из tools.jackson.databind.
